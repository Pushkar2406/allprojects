...
using HOLDay4.Models;

namespace HOLDay4.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            context.Entry<Account>(a).State = (EntityState)EntityState.Modified;
            context.SaveChanges();
            return RedirectToAction("Index");
        }
        public ActionResult Delete(int? accno)
        {
            var account_to_delete = (from a in context.Accounts
                                     where a.AccountNumber == accno
                                     select a).SingleOrDefault();
            context.Entry<Account>(account_to_delete).State = (EntityState)EntityState.Deleted;
            context.SaveChanges();
            return RedirectToAction("Index");
        }
        [ChildActionOnly]
